...
class OperationsContractTests(unittest.TestCase):
    ...
    def test_compose_profile_separates_api_worker_and_durable_dependencies(self) -> None:
        ...
        self.assertIn("pip install --no-cache-dir \".[postgres,object-storage]\"", dockerfile)
        self.assertIn("EASYTOWING_DATABASE_URL=", env_example)
        self.assertIn("EASYTOWING_BOOTSTRAP_TOKEN=", env_example)
        self.assertIn("EASYTOWING_ARTIFACT_S3_BUCKET=", env_example)
        self.assertIn("EASYTOWING_ARTIFACT_S3_SSE=", env_example)
        self.assertIn("EASYTOWING_ARTIFACT_S3_BUCKET", compose)
